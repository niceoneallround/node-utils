...
describe('7 restServer TLS Tests', function () {
  'use strict';
  ...
  before(function (done) {
    restService7 = restServiceFactory.create(props);
    restService7.start(function (err) {
      assert(!err, util.format('Unexpected error starting service2: %j', err));
      done();
    });
  });
  ...
  after(function (done) {
    restService7.stop(function (err) {
      assert(!err, util.format('Unexpected error stopping service: %j', err));
      done();
    });
  });
  ...
  it('7.1 should be all get on HTTPS', function (done) {
    ...
    restService7.registerGETHandler('/path_71', handler71);
